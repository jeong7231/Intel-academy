...
void setup() {
  lcd.init();
  lcd.backlight();
  lcdDisplay(0, 0, lcdLine1); // 월.일 시:분:초
  lcdDisplay(0, 1, lcdLine2); // 수신된 메시지 표시
...
void socketEvent() {
  ...
  //[JTY_ARD]LED@ON : pArray[0] = "JTY_ARD", pArray[1] = "LED", pArray[2] = "ON"
...
void lcdDisplay(int x, int y, char *str) {
  ...
  lcd.print(str);
  for (int i = len; i > 0; i--)
    lcd.write(' '); // 쓰레기값 지우기 남은공간 스페이스로 클리어하겠다~?
}
