...
#define ARR_CNT 5
#define CMD_SIZE 60
char lcdLine1[17] = "Smart IoT By JTY";
char lcdLine2[17] = "";
char sendBuf[CMD_SIZE];
char recvId[10] = "JTY_LIN";  // SQL 저장 클라이이언트 ID
...
*/
 ...
 //   Serial.println(lcdLine2);
    ...
    if(getSensorTime != 0 && !(secCount % getSensorTime)) {
      ...
      sprintf(sendBuf,"[%s]SENSOR@%d@%s@%s\r\n","JTY_SQL",cds,tempStr,humiStr); //내가 추가함 (SQL 연동 안되길래)
      BTSerial.write(sendBuf);   
    }    
...
//    sprintf(sendBuf, "[%s]%s@%s\n", pArray[0], pArray[1], pArray[2]);
    sprintf(sendBuf, "[JTY_SQL]SETDB@%s@%s@%s\n",pArray[1],pArray[2],pArray[0]);
  }
  ...
  else if(!strcmp(pArray[1],"GETSENSOR"))
  {
    ...
    //sprintf(sendBuf,"[%s]SENSOR@%d@%s@%s\r\n","JTY_SQL",cds,(int)temp,(int)humi); //내가 추가함 (SQL 연동 안되길래)
  }
